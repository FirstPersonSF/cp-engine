...
3. From inside the cp tenant clone, run `cxp link-local`. This writes a `.cp-link` file into each configured source repo (containing the absolute path of its cp working dir) and adds `.cp-link` to that repo's `.git/info/exclude`.
...
Pre-v0.7 layouts had an extra `projects/` segment under each scope; `cxp migrate-projects-flat` moves them to the v0.7 shape in place.
