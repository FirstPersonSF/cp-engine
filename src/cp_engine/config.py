...
class EngineVersionMismatch(ConfigError):
    ...
    def __init__(self, installed: str, required: str) -> None:
        ...
        super().__init__(
            ...
            "    published release tag). Resolve the pin to a real tag first —\n"
            "    the constraint itself is not one:\n"
            "      TAG=$(cxp resolve-engine-pin)   # e.g. v0.120.4\n"
            "      uv tool install --force --reinstall \\\n"
            "        --from \"git+https://github.com/FirstPersonSF/cp-engine.git@$TAG\" \\\n"
            "        cp-engine\n"
            "  - You DO have a clone (engine development):\n"
            "      uv tool install --force --reinstall --from <clone-path> cp-engine\n\n"
            "Then update the plugin too — the two halves install separately and\n"
            "drift apart silently:\n"
            "      claude plugin update cp-engine@cp-engine\n\n"
            "Notes:\n"
            "  - `--reinstall` is not optional. Bare `--force` skips the\n"
            "    reinstall when the version string looks unchanged, and\n"
            "    `uv tool upgrade` is a no-op when the receipt pins an exact\n"
            "    rev — both report success and change nothing.\n"
            "  - `claude plugin install` refuses with 'already installed';\n"
            "    `update` is the verb that works.\n\n"
