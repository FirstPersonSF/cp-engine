---
Project: 1P Test
Provenance: Version 0.0.0-golden | 2026-05-13
Filename: master-cp.md
Author: cp-engine (sync)
---

# 1P Test — Master CP

> Single index across First Person Jobs (1P), First Person internal tooling, and Canonic.
> Status, name, owner, and last-touched are automation-managed by cp-engine; do not hand-edit
> inside engine-managed regions (marked by `cp-engine:start` / `cp-engine:end` HTML comments).

<!-- cp-engine:start last-sync-timestamp -->
**Last sync:** 2026-05-13T09:30:00+00:00
<!-- cp-engine:end last-sync-timestamp -->

<!-- cp-engine:start sprint-facts-strip -->
**Total hours** 16.5 · **Tony** 10.5 · **Drew** 6 ·
**Active** 2 ·
**Stale asks** 1 ·
**Escalated** 1 ·
**Decisions due** 1 ·
**Prior sprint** 2026-W19
<!-- cp-engine:end sprint-facts-strip -->

<!-- cp-engine:start agenda -->
## Agenda — what needs attention this week
**Risks needing decision** (1)
- `ggl-5168` Legal turnaround may slip past May 22

**Stale client asks** (1)
- `ggl-5168` Volume forecast from ops team — 12d

**Horizon items maturing** (1)
- `ggl-5168` Whether to renew for Q3 — by W21
<!-- cp-engine:end agenda -->

<!-- cp-engine:start slack-rollup -->
## Recent Slack activity (this sprint week)

<details>
<summary><strong>2 projects with Slack chatter this week</strong></summary>


**`ggl-5168` — Playbooks (Activation)** · [W## sprint](./sprints/2026-W20/ggl-5168.md)
> Tony and Geoff drove a heads-down build week.


**`ibx-5153` — Infoblox AI Campaign** · [W## sprint](./sprints/2026-W20/ibx-5153.md)
> Carol framework deck shipped to Janet.


</details>
<!-- cp-engine:end slack-rollup -->

<!-- cp-engine:start exceptions-summary -->
**Exceptions:** 3 this week — see [`exceptions/`](exceptions/README.md) for unregistered repo activity.
<!-- cp-engine:end exceptions-summary -->

<!-- cp-engine:start active-pipeline -->
## 1P — Pipeline (2)
_Two deals in flight_

Sales-stage progression: Inquiry → Negotiation → Contract.

| Account | Code | Project | Stage | Owner | Budget | Last activity | One-line summary | CP |
|---|---|---|---|---|---|---|---|---|
| [SAP](1p/sap/cp.md) | `sap-5174` | SAP All in with AI | Inquiry | Drew | — | — | Kickoff July 8; workshop July 30. | [CP →](1p/sap/sap-5174/cp.md) · [W20 →](sprints/2026-W20/sap-5174.md) |
| [Pebble Foods](1p/pebble-foods/cp.md) | `peb-5200` | Pebble Foods Discovery | Negotiation | Drew | $45k | — | Pricing counter-proposal in front of Maria. | [CP →](1p/pebble-foods/peb-5200/cp.md) · [W20 →](sprints/2026-W20/peb-5200.md) |
<!-- cp-engine:end active-pipeline -->

<!-- cp-engine:start active-tree -->
## Active workstreams (5)
_Five workstreams in motion_

### Google

| Code | Workstream | Label | Owner | Budget | Last activity | One-line summary | CP |
|---|---|---|---|---|---|---|---|
| `ggl-5168`<br>_Last week: Drew 6h, Tony 2.5h (8.5h total)._ | ggl-5168 Playbooks (Activation) |  | Drew | $80k | — | Storyboards in flight; client review Wed. | [CP →](1p/google/ggl-5168/cp.md) · [W20 →](sprints/2026-W20/ggl-5168.md) |
| `ggl-9998` | ggl-9998 Internal Scratch |  | Drew | — | — |  | [CP →](1p/google/ggl-9998/cp.md) · [W20 →](sprints/2026-W20/ggl-9998.md) |

### Infoblox

| Code | Workstream | Label | Owner | Budget | Last activity | One-line summary | CP |
|---|---|---|---|---|---|---|---|
| `ibx-5153`<br>_Last week: Tony 8h (8h total)._ | ibx-5153 Infoblox AI Campaign |  | Tony | $120k | — | Carol framework deck in client review. | [CP →](1p/infoblox/ibx-5153/cp.md) · [W20 →](sprints/2026-W20/ibx-5153.md) |

### First Person

| Code | Workstream | Label | Owner | Budget | Last activity | One-line summary | CP |
|---|---|---|---|---|---|---|---|
| `1pi-9005-mission-control` | Mission Control | Initiative | Tony | — | — | Workspace IA shipped; integrations registry live. | [CP →](firstpersonsf/1pi-9005-mission-control/cp.md) · [W20 →](sprints/2026-W20/1pi-9005-mission-control.md) |

### Canonic

| Code | Workstream | Label | Owner | Budget | Last activity | One-line summary | CP |
|---|---|---|---|---|---|---|---|
| `cnc-9004-storyos` | StoryOS | Initiative | Drew | — | — | Substrate design in review. | [CP →](canonic/cnc-9004-storyos/cp.md) · [W20 →](sprints/2026-W20/cnc-9004-storyos.md) |
<!-- cp-engine:end active-tree -->

<!-- cp-engine:start last-week-workload -->
## Last week's workload (week of 2026-05-04)

| Person | Total | Engagements | Internal admin |
|---|---|---|---|
| Tony Rossi | **14.5h** | 10.5h (2 proj) | 4h |
| Drew Fiero | **6h** | 6h (1 proj) | — |
<!-- cp-engine:end last-week-workload -->

<!-- cp-engine:start holding-subtable -->
<details>
<summary><strong>Holding</strong> (2)</summary>

| Code | Name | Owner | Last touched | CP |
|---|---|---|---|---|
| `ggl-5136` | Go Safety Website | Drew | 2026-05-12 | [→](1p/google/ggl-5136/cp.md) |
| `1pi-9007-market-scorecard` | Market Scorecard | Tony | 2026-05-12 | [→](firstpersonsf/1pi-9007-market-scorecard/cp.md) |

</details>
<!-- cp-engine:end holding-subtable -->

<!-- cp-engine:start closed-recent -->
<details>
<summary><strong>Closed engagements (last 30 days)</strong> (1)</summary>

- `ibx-5100` — Infoblox Rebrand — closed 2026-05-06


</details>
<!-- cp-engine:end closed-recent -->

## Decisions (cross-cutting, hand-written)

1. _<decision that belongs to no one company — sprint-planning `account_decisions` land here; an account's or program's own decisions live on that node's `cp.md`>_ (_<date>_)
