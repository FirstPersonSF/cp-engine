...
Summary, decisions, notes, stakeholders), **meetings** (`meetings/`,
sprint-file bullets), **players** (stakeholders + the spine's Stakeholders
layer), **work** (the sprint file `sprints/<W##>/<code>.md`), and **scope**
...
| `open workset <code>:<name>` | replace → 4 | Load only that tunnel's live elements (`open_workset`). |
