...
Loaded: `master-cp.md` + `weekly-cp.md` + every active project's `cp.md`
...
- A **Focus list** — the 5–8 projects that need the room, each with a
  one-line reason (decision due / blocker / deadline / slipping); steady
  projects named but not dwelt on
...
ClickUp milestones and the tenant capacity/hours metrics as raw
material.
