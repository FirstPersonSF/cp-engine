...
- **Fix: authored reverse-mirror aborting on retired elements.** `cp sync`'s
  authored reverse-mirror (DB→disk) passed every `origin='authored'` row group
  to `write_authored_element`, including element-level archived (retired)
  elements. A retired element has zero live versions, so it tripped the
  writer's "expected 1 live version" guard, which raised — and because the
  `except` wrapped the whole loop, one retired element **aborted the entire
  project's mirror pass**, silently stranding every later element's mirror
  (sync still exits 0). Any project with a retired authored element stopped
  refreshing its on-disk spine. The account mirror already skipped archived
  elements; the authored loop now does the same: skip archived groups, unlink
  their stale mirror file, and wrap each element's write in its own
  try/except so a single malformed element can't strand the sweep. Regression
  test `test_authored_reverse_mirror_skips_retired_element`.

## v0.74.0 — 2026-07-21

- **Spine steps — an ordered progress trail inside an element (#119).** Four new
  `cp-sources` MCP verbs — `add_spine_step`, `set_spine_step`,
  `reorder_spine_step`, `remove_spine_step` — author a lightweight ordered trail
  of "moves" inside any spine element (drafted → ratified → rewriting → booked).
  A step is a marker + optional note (`{position, title, status ∈
  done|active|upcoming, step_date?, note?}`), NOT a version/source/body — it lives
  in its own MC-2 table `spine_steps` (mig 119), keyed by
  `(project_id, est_item_id)`, ordered by an explicit integer `position` (ties
  break by `step_date`). Verbs resolve the parent element by the usual key
  discipline (est_item_id or a distinct framing substring) and scope every write
  to `(project_id, est_item_id)` so a stray id can't touch another element's
  trail. **A step never writes the work-item's schedule `done`** — the
  all-steps-done → suggest-complete bridge is a UI affordance (reusing
  `suggest_done`), never an auto-write. Steps mirror to `spine/_authored/<slug>.md`
  frontmatter on `cp sync` (round-trips through `parse_substance`); the MC-2
  outline API attaches them per element for the card trail / focus timeline /
  Journey map. Design: `cp/docs/plans/2026-07-21-spine-steps-build.md`.

## v0.73.0 — 2026-07-21

- **Document comments — phase 2 + docx anchors (#108 complete).** Two parts
  finishing the read-comments capability. (1) `pull_document_comments` (and the
  offline extractor) now resolve **`anchored_text` for .docx** — the text a
  comment sits on — by parsing `word/document.xml` `commentRangeStart/End`
  markers (was null for docx; xlsx and Google Docs already carried it). Handles
  overlapping ranges; unpaired markers leave it null, never raises. Verified
  live: 102/113 anchors on ibx-5153's "Our AI Story". (2) **Ingest-layer
  extraction** via re-pinning `1p-component-library` (`document-ingest` +
  co-versioned refs) `34f876d` → `3ea70cc`: the docx/pptx/xlsx parsers now append
  a `## Comments` section (author · anchor · text · replies) to the parsed body,
  so reviewer comments become **searchable in RAG on re-ingest** — the highest-
  signal stakeholder feedback in a reviewed doc was previously thrown away.
  Deploy the webhook to pick up the ingest change in prod.

## v0.72.0 — 2026-07-21

- **New `pull_document_comments` MCP verb — read reviewer comments on any
  document (#108).** Inline comments/annotations are the highest-signal, lowest-
  effort stakeholder capture on a project — a client's own reviewers saying, in
  their words, what to change — but the ingest parsers drop them entirely (they
  live in the file's comment layer, not its body text). The verb reads them LIVE:
  a Google-Drive-hosted doc via the Drive API `comments.list` (the ONLY way to
  reach Google-Docs comments, which exist in no export), otherwise by fetching
  the original Office binary and parsing its comment XML — `word/comments.xml`,
  `ppt/comments/*`, `xl/threadedComments/*` — offline via the new
  `cp_engine/doc_comments.py` (no python-docx/pptx dependency; author names
  resolved). Returns `{title, provider, comment_count, comments}` where each
  comment is `{author, date, anchored_text, comment, replies[]}`; grouped by
  author it doubles as stakeholder intelligence. Tool manifest 27 → 28. Restart
  the `cp-sources` MCP to pick it up. The ingest-parser half (append comments to
  the searchable body on re-ingest) is #108's second phase, not yet shipped.

## v0.71.0 — 2026-07-21

- **New `create_note` MCP verb — leave a partner a Note from an agent session
  (#107).** MC-2's Notes feature (in-app unread + Slack DM) had no `cp-sources`
  verb, so an agent finishing session work could only draft markdown for the
  human to paste. `create_note(project_code, recipient, body, author?)` resolves
  `recipient`/`author` to ONE entity by display-name substring ("Marcello") or
  email (`author` defaults to the acting partner), then reproduces the dashboard
  endpoint's two effects via the same MC-2 service key the other write verbs use:
  inserts the `notes` row (`status='unread'`) and delivers the Slack DM (reusing
  `slack.post_dm`; Block Kit format mirrored from mc-2 `notes_slack.py`). The
  HTTP endpoint needs a Supabase JWT the MCP doesn't have, so this takes the
  service-key path instead. Delivery is best-effort (`slack_delivery`:
  sent|failed|skipped); the note lands in-app regardless. Author it only when the
  human asks to notify a partner. New `cp_engine/notes.py`; `Tables` registry
  gains `ENTITIES` + `NOTES`; tool manifest 26 → 27. Restart the `cp-sources` MCP
  server to pick up the verb. The DM block format is duplicated from mc-2; #107
  tracks consolidating it.

## v0.70.1 — 2026-07-20

- **Fix: `pull_project_source` with a `query` reported "no source named …" for a
  doc that exists.** The scoped-chunk RPC returns only the top `limit` chunks
  across the whole project scope; with a `query` those are vector-ranked, so a
  document whose chunks rank below the top `limit` for that query — a short
  spreadsheet against many semantically-closer PDFs, e.g. ibx-5192's
  `Infoblox_Solutions_Framework_Use_Cases_Capabilities.xlsx` — dropped out of the
  window and was reported as not-found, even though its title is in the manifest.
  The miss-widen retry (already present for `query=None`) now fires on the query
  path too, keeping the query embedding so a successful widen still returns
  query-ranked chunks. Resolving a doc BY TITLE no longer depends on how its
  chunks rank against unrelated docs. Restart the `cp-sources` MCP server to pick
  this up.

## v0.70.0 — 2026-07-20

- **Spine batch retire (#105).** New MCP verb `retire_spine_elements(code,
  keys)` retires a list of elements in one call — each exactly as
  `retire_spine_element` (archive every version, supersede the live row, cascade
  typed edges #96). Slot cleanups that collapse many raw cards are now one
  operation instead of N. Per-key results (`{key, est_item_id, retired,
  edges_removed}` for hits, `{key, note}` for misses) so a resolution miss
  doesn't fail the batch. The single-element body was extracted to a shared
  `_retire_one` helper. Pairs with the mc-2 workbench bulk-bar Retire action.
- **Spine element-as-provenance (#104).** New MCP verbs
  `add_element_provenance(code, key, source_key)` / `remove_element_provenance`
  attach ANOTHER spine element as provenance to a card — the tiering-rule move
  for "this synthesis card absorbs these raw cards". Where `add_element_source`
  attaches an ingested rag_asset, this attaches a spine element as a typed
  `{type: spine_element, id, title, retired}` link. The source element MAY be
  already-retired (the usual cleanup case) — the link is a property of the
  surviving card, so it **survives the source's retirement**, closing the
  lineage hole the ibx-5192 synthesis cleanup exposed (previously the only move
  was retire-and-lose-the-link). New `modify_element_provenance` +
  `_resolve_source_element` (a retire-tolerant source resolver) in
  `project_sources`. Source dedup keys on `(type, id)` so an element source
  never collides with a rag_asset of the same id.
- Tool manifest 23 → 26. `CLAUDE.md` template documents the three new verbs.
  These are read/write MCP surface changes only — no schema migration, no
  webhook change. Restart the `cp-sources` MCP server to pick up the new verbs.

## v0.69.5 — 2026-07-20

- **Strip NUL bytes from ingested text.** A NUL (U+0000) in decoded text parses
  and embeds fine but fails the Postgres insert (`22P05:   cannot be
  converted to text`), dropping the whole document. The cp1252/latin-1 encoding
  fallback (v0.69.3) can surface a raw `0x00` byte as `U+0000`, which reached
  the DB. `read_text_file` now strips NUL on every decode path — only NUL;
  tabs/newlines and other control chars are legitimate and store fine. Fixed
  upstream in `1p-component-library` (#11); this release re-pins to `@34f876d`.
  A storage-layer catch-all for NUL from non-text parsers is tracked separately
  (component #12). Deploy the webhook to pick this up in prod ingest.
