...
```
   ...
   - The cp CLI                      - <scope>/<dir>/cp.md (mixed)      - everything (mode 4)
...
| **Warm** | `master-cp.md` + `weekly-cp.md` + each project CP (`<scope>/<dir_slug>/cp.md`). Loaded per session via mode rules. | Updated every partners' review + ad-hoc during the week. Word-count gated. |
...
3. Reconcile into the engine-managed region of `<scope>/<dir_slug>/cp.md`
...
| **2. Single-project** | `master-cp.md` + `<scope>/<dir_slug>/cp.md` | Other project CPs, `weekly-cp.md` | `update <code>`, `check status <code>`, opening a session in a tracked project repo (auto-loads its CP) |
...
- `also load <code>` → adds `<scope>/<dir_slug>/cp.md` to whatever's already loaded; remains mode 2-ish
...
| `update <code>` | 2 | Open `<scope>/<dir_slug>/cp.md` for editing. |
| `check status <code>` | 2 | Read `<scope>/<dir_slug>/cp.md`; summarize without editing. |
...
| `also load <code>` | additive | Layer `<scope>/<dir_slug>/cp.md` onto current mode. |
| `switch to <code>` | replace → 2 | Discard previous mode; load `<scope>/<dir_slug>/cp.md`. |
...
| **Project CP** | One file per project: `<scope>/<dir_slug>/cp.md`. |
