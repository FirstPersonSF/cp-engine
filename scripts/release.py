...
reporting it, a reader asking "what version is running?" got a precise
wrong answer. Anything a human has to remember to bump will eventually
read like that one did.
...
HOSTED_SERVER = REPO_ROOT / "prototypes" / "hosted-mcp" / "server.py"
CHANGELOG = REPO_ROOT / "CHANGELOG.md"
# Not written by this script, but REGENERATED by the `uv run` test/build steps
# below: uv re-resolves the workspace and rewrites cp-engine's own `version`
# entry to match the freshly-bumped pyproject. Left unstaged it surfaces as a
# dirty tree straight after a "successful" release (observed on v0.96.0,
# v0.97.0 and v0.97.1 — hand-committed each time), and the NEXT release then
# aborts its clean-tree pre-flight for a reason that has nothing to do with
# that release.
...
def bump_hosted_server_version(new: str) -> None:
    ...
    WHY THIS IS HERE (2026-09-17). The constant was hand-maintained and read
    `hosted-cp-spike/0.0.6` at engine 0.120.1 — nine months and ~90 releases
    stale. That was survivable while it was an internal label, and stopped
    being survivable the moment `whoami` reported it: a field named
    `server_version` sitting next to an accurate `build` hash is a trap,
    because a reader asking "what version is running?" gets a confident,
    precise, wrong answer. Observed exactly that, in a fresh session, the day
    the field shipped.

    Deriving it here rather than trusting a human to remember is the whole
    point — the file this writes is the one a reader reaches for when
    something looks wrong, so it must not be the one that drifts.

    `build` in `/health` and `whoami` stays the load-bearing field: it hashes
    what is actually loaded in the container, so it catches a deploy that
    never happened. This makes the two agree instead of contradicting.
    """
    text = HOSTED_SERVER.read_text()
    pattern = re.compile(r'SERVER_VERSION = "[^"]*"')
    if not pattern.search(text):
        raise ReleaseError(
            f'{HOSTED_SERVER} has no `SERVER_VERSION = "..."` line to update.'
        )
    HOSTED_SERVER.write_text(
        pattern.sub(f'SERVER_VERSION = "hosted-cp/{new}"', text, count=1)
    )
